...
import { 
  ...
  Award, Quote, Calculator, Gift, RotateCcw
} from 'lucide-react';
...
export default function CalculadoraTributos() {
  ...
  // Lógica da Calculadora Comum (Segura, livre de aviso do eval direto)
  ...
  const calculateResult = () => {
    try {
      ...
      // Sanitização rígida para avaliar apenas operações matemáticas
      const cleanExpr = sanitized.replace(/[^-()\d/*+.]/g, '');
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 px-0 sm:px-6 pb-12 font-sans mx-[-3px] sm:mx-auto">
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-[20px] gap-1.5 max-w-sm mx-auto shadow-inner border border-slate-200/50">
        <button 
          onClick={() => setAbaAtiva('tributos')}
          className={`flex-1 py-2.5 text-xs font-black uppercase tracking-widest rounded-xl transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer ${
            abaAtiva === 'tributos' 
      ...
      {abaAtiva === 'tributos' ? (
        ...
        /* Calculadora Comum com Design Glassmorphism Elegante */
        <div className="max-w-sm mx-auto bg-slate-950 rounded-[32px] p-6 border border-slate-800 shadow-2xl space-y-6">
          <div className="bg-black/60 rounded-2xl p-6 border border-white/5 text-right overflow-hidden shadow-inner flex flex-col justify-end min-h-[96px]">
            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Visor</span>
            <div className="text-3xl font-extrabold text-white truncate tracking-tight">{calcDisplay}</div>
          </div>

          <div className="grid grid-cols-4 gap-3">
            <CalcBtn label="C" color="bg-rose-950/40 text-rose-400 border-rose-900/20" onClick={() => setCalcDisplay('0')} />
            <CalcBtn label="÷" color="bg-blue-900/30 text-blue-400 border-blue-800/20" onClick={() => pressDigit('÷')} />
            <CalcBtn label="×" color="bg-blue-900/30 text-blue-400 border-blue-800/20" onClick={() => pressDigit('×')} />
            <CalcBtn label="⌫" color="bg-slate-900 text-slate-300 border-slate-800" onClick={() => setCalcDisplay(prev => prev.length > 1 ? prev.slice(0, -1) : '0')} />
            ...
            <CalcBtn label="=" color="bg-emerald-500 text-slate-950 border-emerald-400 shadow-md shadow-emerald-500/10" onClick={calculateResult} />
...
function CalcBtn({ label, color = 'bg-slate-900 text-slate-300 border-slate-800/80', span = '', onClick }) {
  return (
    <button 
      type="button"
      onClick={onClick}
      className={`${span} ${color} py-4 rounded-xl text-lg font-black border transition-all active:scale-95 hover:brightness-110 cursor-pointer flex items-center justify-center shadow-xs`}
    >
      {label}
    </button>
  );
