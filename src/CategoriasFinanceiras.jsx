        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
            <Card className={`p-0 ${abaAtivaMobile !== 'receita' ? 'hidden md:block' : 'block'}`}>
              ...
              <TabelaSimples lista={categorias.filter(c => c.tipo === 'receita')} onEdit={prepararEdicao} onDelete={removerCategoria} cor="emerald" podeEditar={podeEditar} />
            </Card>
            <Card className={`p-0 ${abaAtivaMobile !== 'despesa' ? 'hidden md:block' : 'block'}`}>
              ...
              <TabelaSimples lista={categorias.filter(c => c.tipo === 'despesa')} onEdit={prepararEdicao} onDelete={removerCategoria} cor="rose" podeEditar={podeEditar} />
            </Card>
          </div>
        </div>
      </div>
...
function TabelaSimples({ lista, onEdit, onDelete, cor, podeEditar }) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="table-mib">
        <thead><tr><th>Nome / Descrição</th>{podeEditar && <th className="text-right">Ação</th>}</tr></thead>
        <tbody>
          {lista.map(cat => (
            <tr key={cat.id}>
              <td className="py-3">
